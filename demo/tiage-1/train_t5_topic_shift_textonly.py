...
def build_samples(rows: List[dict], context_turns: int) -> Dict[str, List[Sample]]:
    ...
    for (split, _dialog_id), turns in by_dialog.items():
        ...
        for i, cur in enumerate(turns):
            ...
            context_text = " </s> ".join([h["text"] for h in hist]) if hist else "<no_context>"
            text = (
...
class T5TextOnlyClassifier(nn.Module):
...
def main() -> None:
    parser = argparse.ArgumentParser(description="TIAGE topic shift detection with T5 text-only baseline")
    parser.add_argument("--nodes_csv", type=str, default="demo/tiage-1/outputs_nodes/tiage_anno_nodes_all.csv")
    parser.add_argument("--output_dir", type=str, default="demo/tiage-1/results_t5_textonly")
    ...
    print(f"[INFO] model={args.model_name} context_turns={args.context_turns} max_length={args.max_length}")

    rows = parse_nodes(nodes_csv)
    split_samples = build_samples(rows, args.context_turns)
    for split in ("train", "dev", "test"):
        print(f"[INFO] {split} samples={len(split_samples[split])}")

    tokenizer = AutoTokenizer.from_pretrained(args.model_name)
    model = T5TextOnlyClassifier(args.model_name, dropout=args.dropout)
    ...
    metrics_rows = [
        {"split": "train", "best_epoch": best_epoch, "best_threshold": best_threshold, "model_name": args.model_name, **train_metrics},
        {"split": "dev", "best_epoch": best_epoch, "best_threshold": best_threshold, "model_name": args.model_name, **dev_metrics},
        {"split": "test", "best_epoch": best_epoch, "best_threshold": best_threshold, "model_name": args.model_name, **test_metrics},
    ]
    save_csv(
        output_dir / "metrics.csv",
        metrics_rows,
        ["split", "best_epoch", "best_threshold", "model_name", "precision", "recall", "macro_f1", "accuracy", "size"],
    )
    save_csv(
        output_dir / "predictions.csv",
        train_rows + dev_rows + test_rows,
        ["split", "dialog_id", "turn_id", "node_id", "label", "pred", "prob_1"],
    )
